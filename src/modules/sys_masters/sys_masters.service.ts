...
@Injectable()
      export class SysMastersService {
       ...
       async create(createSysMastersDto: CreateSysMastersDto, payload: any) {
          ...
          const thisTableInfo = await this.sysTables.findOne({where: { table_name: 'sys_masters',is_active:true, }});
    ...
    async findAll(page: number, size: number, field: string, search: string,payload: any) {
      ...
      const response = this.helpers.getPagingData(data, page, limit,'sys_masters');
      return response || {};
  ...
  async update(id: number, updateSysMastersDto: UpdateSysMastersDto,payload: any) {   
      const thisTableInfo = await this.sysTables.findOne({
        where: { table_name: 'sys_masters',is_active:true, },
      });
      if (!thisTableInfo) throw new ForbiddenException();
