...
@Injectable()
    ...
    async findAll(page: number, size: number, field: string, search: string) {
      ...
      const response = this.helpers.getPagingData(data, page, limit,'sys_role_table');
      return {
        error: false,
        statusCode: 200,
        message: 'Success!',
  ...
  async update(id: number, updateSysRoleTableDto: UpdateSysRoleTableDto,payload: any) {
    try {
