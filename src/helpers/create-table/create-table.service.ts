...
`;
    ...
    const createDtoData = `export class Create${tableName}Dto {\n`;

    const dtoFieldString = '';

    for (let i = 0; i < fieldList.length; i++) {
      if (fieldList[0].primaryKey) continue; // no need to enter primary key as it is autogenerated
  ...
  async createUserDto(dtoName: string, dtoData: string, dtoPath: string) {
    const newDto = await this.createFile(
      dtoPath,
      `${dtoName}.service.ts`,
      dtoData,
    );
